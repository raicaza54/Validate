...
class Process {
    ...
    public function __construct(Fuzz $fuzz = null) {
        ...
        $this->getString = function ($x) {
            return $x[0];
        };
        $this->getScore = function ($x) {
            return $x[1];
        };
    }
    ...
    public function extract($query, $choices = [], $processor = null, $scorer = null, $limit = 5) {
        ...
        $processor = $processor ?: function ($str) {
            return Utils::fullProcess($str);
        };
        $scorer = $scorer ?: function ($s1, $s2) {
            return $this->fuzz->weightedRatio($s1, $s2);
        };

        if (!is_callable($processor)) {
            throw new \InvalidArgumentException('processor is not callable.');
        }
        if (!is_callable($scorer)) {
    ...
    public function extractBests($query, $choices = [], $processor = null, $scorer = null, $cutoff = 0, $limit = 5) {
        ...
        return $bestList->filter(function ($x) use ($cutoff) {
                    return $x[1] >= $cutoff;
                });
    }
    ...
    public function dedupe($containsDupes = [], $threshold = 70, $scorer = null) {
        ...
        foreach ($containsDupes as $item) {
            ...
            $filtered = $matches->filter(function ($x) use ($threshold) {
            ...
            if ($filtered->count() === 1) {
                ...
                $filtered = $filtered->multiSort(
                        $filtered->map(function ($x) {
                            return strlen($x[0]);
                        })->toArray(), SORT_DESC, SORT_NUMERIC, $filtered->map($this->getScore)->toArray(), SORT_DESC, SORT_NUMERIC, $filtered->map($this->getString)->toArray(), SORT_ASC, SORT_STRING | SORT_FLAG_CASE
                );
